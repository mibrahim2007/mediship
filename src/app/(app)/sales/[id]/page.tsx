...
import { getSalesOrderById } from "@/lib/db/sales"
import { notFound } from "next/navigation"
import Link from "next/link"
import { ArrowLeft, Printer, Pencil } from "lucide-react"
import { formatCurrency, formatDate } from "@/lib/utils"
import { SalesOrderActions } from "@/components/app/sales-order-actions"
...
export default async function SalesOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const order = await getSalesOrderById(params.id, session.companyId!)
  if (!order) notFound()
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          {["quotation", "sales_order"].includes(order.status) && (
            <Link
          ...
          <SalesOrderActions orderId={order.id} currentStatus={order.status} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { label: "Customer", value: (order as any).contacts?.name ?? "—" },
          { label: "Order Date", value: formatDate(order.order_date) },
          { label: "Expiry Date", value: order.expiry_date ? formatDate(order.expiry_date) : "—" },
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              ...
              <th className="text-right px-5 py-2 text-slate-500 font-medium">Unit Price</th>
              <th className="text-right px-5 py-2 text-slate-500 font-medium">Disc%</th>
          ...
          <tbody>
            {order.lines.map((l: any, i: number) => (
              <tr key={l.id} className="border-b border-slate-50">
                ...
                <td className="px-5 py-3 text-right text-slate-500">{l.discount ? `${l.discount}%` : "—"}</td>
